...
import { useCollaboration } from "@st/hooks/useCollaboration";
...
const collaborateProps = {
  upload: knowledgeProps.upload,
  ai: knowledgeProps.ai,
}

const collaborationConfig = computed(() => {
  if (currentDemo.value !== 'collaboration' || !editable.value) {
    return null
  }
  return {
    documentId: 'speed-tiptap-demo-doc',
    url: 'ws://localhost:3005/collaboration',
    token: globalStore.openJwt ? localStorage.getItem('speed-tiptap-token') : 'speed-test-token',
    user: {
      id: 'demo-user',
      username: 'ykx',
      nickname: '示例用户',
...
const { ydoc, provider } = useCollaboration({
  ...
  enabled: computed(() => currentDemo.value === 'collaboration' && editable.value),
})

const collaborationUser = computed(() => collaborationConfig.value?.user ?? null)

const collaborationReady = computed(() => {
  if (currentDemo.value !== 'collaboration') return true
...
<template>
  ...
  <div class="px-2 h-[600px]">
    ...
    />
    <SpeedTiptapEditor
      v-else-if="currentDemo === 'collaboration' && collaborationReady"
      :editable="editable"
      v-model:title="title"
      preset="knowledge"
      :ydoc="editable ? ydoc : undefined"
      :provider="editable ? provider : undefined"
      :collaboration-user="collaborationUser"
      v-bind="collaborateProps"
    />
  </div>

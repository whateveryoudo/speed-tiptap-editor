  ...
  <teleport :to="getContainer()">
    <div
      ...
      <a-modal
        ...
        :style="{ height: fullscreenModel ? '100%' : getCompatUnit(height) }"
        @ok="emit('ok')"
        @cancel="emit('cancel')">
        <template #title>
          <slot name="title">
        ...
        <template #closeIcon>
          <slot name="closeIcon">
            <a-space class="ant-modal-operate" @click.stop>
              <template v-if="canIchangeSize">
...
watch(visibleModel, async val => {
  centerModal()
  if (props.canIdrag && ((val && Object.is(inited, false)) || props.destroyOnClose)) {
    initDrag()
  }
})
